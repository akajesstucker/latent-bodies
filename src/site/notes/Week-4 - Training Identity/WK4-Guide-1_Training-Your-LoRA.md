...
- [Civitai](/notes/week-4/civitai-lora-training/) **(recommended)** — web-based, no setup, works immediately. Start here if you have no local GPU.
- [Kohya SS](/notes/week-4/kohya-lora-training/) **(local GPU only)** — more control over training. Requires a local NVIDIA GPU (RTX 3080 or better) and a local install. Cloud-hosted Kohya is not reliably working at this time.
