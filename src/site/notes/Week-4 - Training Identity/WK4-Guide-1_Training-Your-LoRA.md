...
- [[WK4-Guide-2_Civitai-Lora-Training|Civitai]] **(recommended)** — web-based, no setup, works immediately. Start here if you have no local GPU.
- [[WK4-Guide-3_Kohya-Lora-Training|Kohya SS]] **(local GPU only)** — more control over training. Requires a local NVIDIA GPU (RTX 3080 or better) and a local install. Cloud-hosted Kohya is not reliably working at this time.
